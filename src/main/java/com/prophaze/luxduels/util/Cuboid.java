...
public class Cuboid implements Iterable<Block>, Cloneable, ConfigurationSerializable {
    ...
    public Cuboid(final World world, final int x1, final int y1, final int z1, final int x2, final int y2, final int z2) {
        ...
        this.y1 = normalize(Math.min(y1, y2));
        this.y2 = normalize(Math.max(y1, y2));
    ...
    public Cuboid(final Location center, int size) {
        this.worldName = "world";

        size = size / 2;
        this.x1 = center.getBlockX() - size;
        this.y1 = center.getBlockY() - size;
        this.z1 = center.getBlockZ() - size;

        this.x2 = center.getBlockX() + size;
        this.y2 = center.getBlockY() + size;
        this.z2 = center.getBlockZ() + size;
    }

    public Cuboid(final World world, final Location center, int size) {
        this.worldName = world.getName();
    ...
    public Cuboid(final String worldName, final Location center, int size) {
        this.worldName = worldName;
    ...
    public Cuboid(final String worldName, final int x, final int y, final int z, int size) {
        this.worldName = worldName;
    ...
    private int normalize(final int y) {
        return Math.max(y, 0);
    }
    ...
    public List<Block> getBlocks() {
        final Iterator<Block> blocks = this.iterator();
        final List<Block> copy = new ArrayList<>();
        while (blocks.hasNext()) copy.add(blocks.next());
        return copy;
    }
    ...
    public Cuboid getLayer(final int axis, final CuboidDirection dir, final int... dirs) {
        if (dir.equals(CuboidDirection.Horizontal))
            return new Cuboid(this.worldName, this.getLowerX(), axis, this.getLowerZ(), this.getUpperX(), axis, this.getUpperZ());
        else if (dir.equals(CuboidDirection.Vertical)) {
            if (dirs[0] == 0) {
                return new Cuboid(this.worldName, axis, this.getLowerY(), this.getLowerZ(), axis, this.getUpperY(), this.getUpperZ());
            } else {
                return new Cuboid(this.worldName, this.getLowerX(), this.getLowerY(), axis, this.getUpperX(), this.getUpperY(), axis);
            }
        }
        return null;
    }

    public Cuboid flatten(final CuboidDirection dir) {
        final int y;

        final int x1 = this.getLowerX();
        final int x2 = this.getUpperX();

        final int z1 = this.getLowerZ();
        final int z2 = this.getUpperZ();

        if (dir.equals(CuboidDirection.Up)) y = this.getUpperY();
        else y = this.getLowerY();

        return new Cuboid(this.worldName, x1, y, z1, x2, y, z2);
    }
     ...
     *
     * @param material - The block type to check for
    ...
    public boolean containsOnly(final Material... materials) {
        final List<Material> mats = Arrays.asList(materials);
        for (final Block b : this) {
            if (!mats.contains(b.getType())) return false;
        }
        return true;
    }
    ...
    public List<Chunk> getChunks() {
        final List<Chunk> res = new ArrayList<>();
